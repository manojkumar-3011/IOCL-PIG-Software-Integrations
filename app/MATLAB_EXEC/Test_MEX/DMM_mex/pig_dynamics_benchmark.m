function x_next = pig_dynamics_benchmark(t,x,u,params)
...
% main  
n_omega_ie = @(p) rotn_factr_en*[rot_rate_of_earth*cosd(p(1));0;rot_rate_of_earth*sind(p(1))]; 
n_omega_en = @(p,v) manifld_factr_en*[-v(2)/((east_radius_of_earth+p(3)));v(1)/(north_radius_of_earth+p(3));-v(2)*tand(p(1))/(east_radius_of_earth+p(3))]; 
...
gl = @(p) -Sx(n_omega_ie(p))*Sx(n_omega_ie(p))*[0;0;(east_radius_of_earth+p(3))]+[0;0;-earth_gravity]; 

G_func = @(p,v) [180*v(1)/(pi*(north_radius_of_earth+p(3)));...
    -180*v(2)/(pi*(east_radius_of_earth+p(3))*cosd(p(1)));v(3)]; 
