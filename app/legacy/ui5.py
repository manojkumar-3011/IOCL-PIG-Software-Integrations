...
from schema import PipelineGain, ExecutionMode
from constants import (
    APP_TITLE, COMPANY_NAME, LOGO_PATH, DEFAULT_MARKER_IDS, DEFAULT_ADMIN_MARKER_IDS,
    DEFAULT_PIPELINE_GAIN, DEFAULT_EXECUTION_MODE, DEFAULT_OUTPUT_RESET
)
from messages import (
    SUBMIT_SUCCESS_MESSAGE, RESET_MESSAGE, INVALID_MARKER_IDS_ERROR,
    MARKER_IDS_PROMPT, ADMIN_MARKER_IDS_PROMPT, OUTPUT_RESET_PROMPT
)
...
class EstimationConfigUI(QWidget):
    ...
    def initUI(self):
        ...
        image_path = os.path.join(current_dir, LOGO_PATH)
        ...
        markers_layout.addWidget(QLabel(MARKER_IDS_PROMPT))
        self.marker_ids = QLineEdit()
        self.marker_ids.setPlaceholderText(DEFAULT_MARKER_IDS)
        markers_layout.addWidget(self.marker_ids)

        markers_layout.addWidget(QLabel(ADMIN_MARKER_IDS_PROMPT))
        self.admin_marker_ids = QLineEdit()
        self.admin_marker_ids.setPlaceholderText(DEFAULT_ADMIN_MARKER_IDS)
        ...
        self.pipeline_gain.addItems([gain.value for gain in PipelineGain])
        self.pipeline_gain.setCurrentText(DEFAULT_PIPELINE_GAIN.value)
        ...
        self.quick_mode = QRadioButton(ExecutionMode.QUICK.value)
        self.detailed_mode = QRadioButton(ExecutionMode.DETAILED.value)
        self.quick_mode.setChecked(DEFAULT_EXECUTION_MODE == ExecutionMode.QUICK)
        ...
        self.output_reset = QCheckBox(OUTPUT_RESET_PROMPT)
        self.output_reset.setChecked(DEFAULT_OUTPUT_RESET)
        ...
        branding_label = QLabel(f"भारतीय प्रौद्योगिकी संस्थान मुंबई | Indian Institute of Technology Bombay\nPowered by {COMPANY_NAME}")
    ...
    def submit_action(self):
        ...
        # Validate marker IDs (you might want to implement more robust validation)
        if not marker_ids:
            QMessageBox.warning(self, "Invalid Input", INVALID_MARKER_IDS_ERROR)
            return

        # ... rest of your input capturing and processing logic ...

        QMessageBox.information(self, "Submission", SUBMIT_SUCCESS_MESSAGE)

    def reset_action(self):
        # Reset to default values
        self.marker_ids.clear()
        self.admin_marker_ids.clear()
        self.pipeline_gain.setCurrentIndex(DEFAULT_PIPELINE_GAIN.value)
        self.quick_mode.setChecked(DEFAULT_EXECUTION_MODE == ExecutionMode.QUICK)
        self.output_reset.setChecked(DEFAULT_OUTPUT_RESET)
