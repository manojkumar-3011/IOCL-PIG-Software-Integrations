...
def apply_noise_reduction(data, method='bilateral'):
    """Advanced noise reduction techniques (no Gaussian filtering)"""
    if method == 'bilateral':
        ...
        # Default to bilateral if unknown method
        return apply_noise_reduction(data, 'bilateral')

def apply_sharpening(data, strength=1.5):
    """Apply unsharp masking for sharpening (using uniform filter instead of Gaussian)"""
    # Create uniform blur instead of Gaussian blur
    blurred = uniform_filter(data, size=3)
...
def process_image_data(data, enhancement_level="Advanced"):
    if enhancement_level == "Basic":
        # Basic processing - bilateral filtering only
        enhanced = apply_noise_reduction(data, 'bilateral')
        ...
        # Step 1: Advanced denoising (bilateral + median)
        denoised1 = apply_noise_reduction(data, 'bilateral')
        denoised2 = apply_noise_reduction(denoised1, 'median')
...
rawdata_pri3 = np.zeros((primary_sen, samps))
rawdata_sec3 = np.zeros((secondary_sen, samps))
